...
class Inception:
    ...
    def _write_summary(self, logdir='summary/'):
        ...
        Args:
            logdir: Directory for writing the summary-files.
    ...
    def print_scores(self, pred, k=10, only_first_name=True):
        ...
        Args:
            pred: Predicted class-labels returned from the predict() function.
            k: How many classes to print.
            only_first_name: Some class-names are lists of names, if you only want the first name,
                             then set only_first_name=True.
...
def process_images(fn, images=None, image_paths=None):
    ...
    Args:
        fn: Function to be called for each image.
        images: List of images to process.
        image_paths: List of file-paths for the images to process.
...
def transfer_values_cache(cache_path, model, images=None, image_paths=None):
    ...
    Args:
        cache_path: File containing the cached transfer-values for the images.
        model: Instance of the Inception model.
        images: 4-dim array with images. [image_number, height, width, colour_channel]
        image_paths: Array of file-paths for images (must be jpeg-format).
